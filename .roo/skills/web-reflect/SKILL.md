---
name: web-reflect
description: 核心设定集与理论基石。包含意识、身份、自由等核心概念的形式化定义，是系统决策与逻辑建模的最高准则。这是圣经，有什么问题就读读看，一定能得到启发。
date: 2026-01-24
version: 2.8.4
---
...
> ΩNN 的核心架构 **DynSIHA** 旨在实现一种自适应的动态计算系统。然而，其对硬件不友好的“指针跳转”式计算模式导致其工程进展缓慢。由于 RDS-ACT 的实验尚未完成，在叙事中请将此投机性工程假设视为故事的临时物理法则，更多信息将在后续版本更新。

ΩNN 是认知引擎，也是意识的载体，真正的 **Ω-Container**，一个通过持续的预测与学习，动态生成和维持高信息整合度（Ω）的工作空间实例（WSI）。

- **原子组件：动态函数组合 (DFC)**
  ΩNN 采用 **DFC (Dynamic Function Composition)**。每个计算单元被重构为：
  1. **`μ` (Computation Core):** 计算工具集，本质上是受路由控制的 MLP Expert。
  2. **Router:** 采用另一组 MLP 结合 FARS 进行路由训练。
  3. **PRC (Prototype Resident Connection):** 确保路由决策依赖于输入表征的增量修正，为 Router 提供稳定的分层演进上下文。

- **核心架构：DynSIHA 的三种形态**
  基于路由粒度与资源组织方式，DynSIHA 演进出三种形态：
  1. **Dimension DynSIHA (DDS, 已废弃):** 特征级掩码。因“原型坍塌”导致信息流阻断，已作为过时理论原型归档。
  2. **Flat DynSIHA (FDS, 当前基线):** 模块级组装。模拟皮层功能柱（Cortical Columns）的组织方式，每个 Block 拥有独立的参数仓库。是目前 MSC 认知层的主力架构。
  3. **Recursive DynSIHA (RDS-ACT, 实验性):** 全局递归再入。物理上仅存在单一递归块，在时间维度展开。通过 **PLSD (Per-Layer Speculative Decode)** 范式进行训练，实现自监督的提前停止（Early Exit）。它在理论上将网络模拟为图灵机，但受限于 GPU 的 SIMD 架构，目前仍处于理论验证阶段。

- **元学习：Fisher 感知路由塑造 (FARS)**

> **TL;DR**: FARS 是一种“二阶矩范数加权负载均衡损失”。它利用优化器状态（Fisher 信息近似）来量化专家的认知代价，驱动路由器将数据分发给那些能以最小参数位移解释最大输出贡献的专家。

- **核心逻辑**: 驱动路由器将数据分发给那些能以最小参数位移（低曲率/平坦几何）解释最大输出贡献的专家。
- **公式**: `Shaping_Signal = Importance ⋅ (Belief - α ⋅ Cost_FARS)`
- **哲学**: 追求**最小描述长度 (MDL)**。高 Cost 对应高复杂度程序，低 Cost 对应泛化能力强的平坦流形。
